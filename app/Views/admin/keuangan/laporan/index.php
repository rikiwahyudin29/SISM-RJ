...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex gap-4 mb-6 border-b border-slate-200 dark:border-slate-700">
        <button onclick="switchTab('pemasukan')" id="tab-pemasukan" class="px-4 py-2 text-sm font-bold text-blue-600 border-b-2 border-blue-600 active-tab transition-all">Laporan Pemasukan</button>
        <button onclick="switchTab('tunggakan')" id="tab-tunggakan" class="px-4 py-2 text-sm font-bold text-slate-500 hover:text-slate-700 dark:text-slate-400 border-b-2 border-transparent transition-all">Rekap Lunas & Tunggakan</button>
    </div>
    ...
    <div id="panel-pemasukan" class="space-y-6">
        ...
        <div class="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
            <form action="" method="get" class="flex flex-col md:flex-row gap-4 items-end">
                <div class="w-full md:w-auto">
                    ...
                    <select name="filter" class="w-full md:w-40 border-slate-200 dark:border-slate-600 rounded-lg text-sm font-bold dark:bg-slate-900 dark:text-white" onchange="this.form.submit()">
                ...
                <div class="w-full md:w-auto">
                    ...
                    <input type="date" name="start" value="<?= $start ?>" class="w-full border-slate-200 dark:border-slate-600 rounded-lg text-sm font-bold dark:bg-slate-900 dark:text-white">
                </div>
                
                <div class="w-full md:w-auto">
                    <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Sampai Tanggal</label>
                    <input type="date" name="end" value="<?= $end ?>" class="w-full border-slate-200 dark:border-slate-600 rounded-lg text-sm font-bold dark:bg-slate-900 dark:text-white">
                </div>

                <div class="flex gap-2">
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-lg font-bold text-sm shadow-lg shadow-blue-500/30 transition-all">Filter</button>
                    
                    <a href="<?= base_url('admin/keuangan/laporan/cetak_transaksi?start='.$start.'&end='.$end) ?>" target="_blank" class="bg-slate-800 hover:bg-slate-900 text-white px-5 py-2 rounded-lg font-bold text-sm flex items-center gap-2 shadow-lg transition-all">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
                        Cetak PDF
                    </a>

                    <a href="<?= base_url('admin/keuangan/laporan/export_excel?start='.$start.'&end='.$end) ?>" target="_blank" class="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2 rounded-lg font-bold text-sm flex items-center gap-2 shadow-lg shadow-emerald-500/30 transition-all">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-emerald-500 rounded-2xl p-6 shadow-lg shadow-emerald-500/20 text-white">
            ...
            <div class="bg-rose-500 rounded-2xl p-6 shadow-lg shadow-rose-500/20 text-white">
            ...
            <div class="bg-blue-600 rounded-2xl p-6 shadow-lg shadow-blue-500/20 text-white relative overflow-hidden">
                        ...
                        <?php else: ?>
                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30">
                                ...
                                <td class="p-4 text-slate-600 dark:text-slate-300">
                                    ...
                                    <div class="text-xs text-slate-400"><?= esc($p['keterangan']) ?></div>
                                </td>
        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
                ...
                <div class="px-4 pb-4 text-center">
                    <a href="<?= base_url('admin/keuangan/tagihan/kelola/'.$r['id']) ?>" class="block w-full py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 dark:bg-slate-700 dark:hover:bg-slate-600 dark:text-slate-300 font-bold text-xs rounded-lg transition-colors">
                        Lihat Detail Siswa
                    </a>
                </div>
            </div>
