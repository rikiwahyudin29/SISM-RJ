...
<div class="p-4 sm:ml-64 mt-14">
    <h2 class="text-2xl font-bold mb-4 dark:text-white">Tambah Kelas</h2>
    ...
    <form action="<?= base_url('admin/kelas/simpan') ?>" method="post">
        <div class="mb-6">
            ...
            <input type="text" id="nama_kelas" name="nama_kelas" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Contoh: X RPL 1" required>
        </div>
    ...
    <select id="guru_id" name="guru_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required>
        ...
        <?php foreach($gurus as $g): ?>
            <?php 
                // Format nama lengkap dengan gelar untuk dropdown
                $namaLengkap = ($g['gelar_depan'] ? $g['gelar_depan'] . ' ' : '') . 
                               $g['nama_lengkap'] . 
                               ($g['gelar_belakang'] ? ', ' . $g['gelar_belakang'] : '');
            ?>
            <option value="<?= $g['id'] ?>"><?= $namaLengkap ?></option>
        <?php endforeach; ?>
    </select>
</div>
