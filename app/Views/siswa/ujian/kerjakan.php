...
<style>
    ...
    /* Navigasi Grid */
    .nav-grid-btn { width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; border-radius: 8px; font-weight: bold; cursor: pointer; border: 2px solid #e5e7eb; }
    .nav-active { border-color: #2563eb; transform: scale(1.1); box-shadow: 0 0 10px rgba(37,99,235,0.3); }
    .nav-filled { background-color: #22c55e; color: white; border-color: #22c55e; }
    .nav-ragu { background-color: #eab308; color: white; border-color: #eab308; }
    .nav-empty { background-color: white; color: #374151; }
</style>

<div id="warning-overlay" class="fixed inset-0 z-[9999] hidden flex-col justify-center items-center bg-black/95 text-white text-center">
    ...
    <p class="text-gray-300 mb-8 max-w-lg">Anda meninggalkan halaman ujian.</p>
    <button onclick="kembaliKeUjian()" class="px-8 py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl">KEMBALI MENGERJAKAN</button>
</div>
...
<div class="flex h-screen bg-gray-50 dark:bg-slate-900 overflow-hidden" id="exam-container">
    <div class="flex-1 flex flex-col h-full relative">
        <div class="bg-white dark:bg-slate-800 p-4 shadow-md flex justify-between items-center z-20 border-b dark:border-slate-700">
            <div>
                ...
                <span class="text-xs text-gray-500"><?= esc($jadwal->nama_mapel) ?></span>
            </div>
            <div class="flex items-center gap-3">
                <div id="main-timer" class="bg-gray-900 text-white px-4 py-2 rounded-lg font-mono font-bold text-xl shadow">--:--:--</div>
                <button onclick="toggleNav()" class="lg:hidden bg-blue-100 text-blue-600 p-2 rounded-lg"><i class="fas fa-th"></i></button>
            </div>
        </div>
                ...
                ?>
                    <div class="soal-item <?= $index == 0 ? '' : 'hidden' ?>" id="soal-index-<?= $index ?>" 
                        ...
                        <div class="flex justify-between items-start mb-6 border-b pb-4 dark:border-slate-700">
                            <span class="bg-blue-600 text-white w-10 h-10 flex items-center justify-center rounded-lg font-bold text-lg shadow"><?= $index + 1 ?></span>
                            <label class="flex items-center gap-2 cursor-pointer bg-yellow-50 dark:bg-yellow-900/20 px-3 py-1.5 rounded-lg border border-yellow-200 hover:bg-yellow-100 transition-colors">
                                <input type="checkbox" class="w-5 h-5 text-yellow-500 rounded focus:ring-yellow-500" 
                                       id="check-ragu-<?= $index ?>" onchange="setRagu(<?= $index ?>, <?= $idSoal ?>)" <?= $isRagu ? 'checked' : '' ?>>
                        ...
                        <div class="text-lg text-gray-800 dark:text-gray-200 leading-relaxed mb-6">
                            ...
                            <?php if(!empty($s['file_gambar'])): ?>
                                <img src="<?= base_url('uploads/bank_soal/' . $s['file_gambar']) ?>" class="mt-4 max-w-full md:max-w-md rounded-lg shadow-sm">
                            <?php endif; ?>
                        ...
                        <div class="space-y-3">
                            <?php if ($s['tipe_soal'] == 'PG'): ?>
                                <?php foreach($s['opsi'] as $opsi): ?>
                                    <label class="group flex items-center p-4 border-2 rounded-xl cursor-pointer hover:bg-blue-50 dark:hover:bg-slate-800 transition-all <?= (isset($s['id_opsi']) && $s['id_opsi'] == $opsi['id']) ? 'border-blue-500 bg-blue-50 dark:bg-slate-800' : 'border-gray-200 dark:border-slate-600' ?>">
                            ...
                            ?>
                                <?php foreach($s['opsi'] as $opsi): ?>
                                    <label class="flex items-center p-4 border-2 rounded-xl cursor-pointer hover:bg-blue-50 transition-all border-gray-200">
                                        ...
                                        <div class="w-6 h-6 border-2 border-gray-400 rounded-md peer-checked:bg-blue-600 peer-checked:border-blue-600 flex items-center justify-center mr-4"><i class="fas fa-check text-white text-xs opacity-0 peer-checked:opacity-100"></i></div>
                                        <span class="text-gray-700"><?= $opsi['teks_opsi'] ?></span>
                                    </label>
                                <?php endforeach; ?>

                            <?php elseif ($s['tipe_soal'] == 'MENJODOHKAN'): 
                                // --- FITUR MENJODOHKAN LENGKAP ---
                                $savedMatch = json_decode($s['jawaban_siswa'] ?? '{}', true);
                                $pilihanKanan = []; foreach($s['opsi'] as $o) { $pilihanKanan[] = $o['teks_opsi']; } shuffle($pilihanKanan); 
                            ?>
                                <div class="bg-blue-50 dark:bg-slate-700/50 p-5 rounded-xl border border-blue-200 dark:border-slate-600">
                                    <p class="text-sm font-bold text-blue-800 dark:text-blue-300 mb-4 flex items-center"><i class="fas fa-link mr-2"></i> Pasangkan:</p>
                                    <div class="grid gap-3">
                                        <?php foreach($s['opsi'] as $pair): ?>
                                            <div class="flex flex-col sm:flex-row sm:items-center justify-between bg-white dark:bg-slate-800 p-3 rounded-lg border border-gray-200 dark:border-slate-600 shadow-sm">
                                                <div class="w-full sm:w-1/2 font-medium text-gray-700 dark:text-gray-300 mb-2 sm:mb-0 pr-2"><?= $pair['kode_opsi'] ?></div>
                                                <div class="w-full sm:w-1/2 relative">
                                                    <select class="w-full p-2.5 bg-gray-50 dark:bg-slate-700 border border-gray-300 dark:border-slate-500 text-gray-900 dark:text-white text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block match-select-<?= $idSoal ?>"
                                                        ...
                                                        <?php foreach($pilihanKanan as $pk): ?>
                                                            <option value="<?= $pk ?>" <?= (isset($savedMatch[$pair['kode_opsi']]) && $savedMatch[$pair['kode_opsi']] == $pk) ? 'selected' : '' ?>><?= $pk ?></option>
                                                        <?php endforeach; ?>
                                                    </select>
                                                </div>
                                            </div>
        ...
        <div class="bg-white dark:bg-slate-800 p-4 border-t dark:border-slate-700 flex justify-between items-center z-20">
            <button onclick="prevSoal()" id="btn-prev" class="px-6 py-2.5 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-lg font-bold disabled:opacity-50" disabled><i class="fas fa-arrow-left mr-2"></i> Prev</button>
            <button onclick="nextSoal()" id="btn-next" class="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold flex items-center">Next <i class="fas fa-arrow-right ml-2"></i></button>
            <button onclick="cekSelesai()" id="btn-selesai" class="hidden px-6 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-lg font-bold flex items-center"><i class="fas fa-check-circle mr-2"></i> Selesai</button>
        </div>
    </div>
    ...
    <div id="nav-sidebar" class="fixed inset-y-0 right-0 w-72 bg-white dark:bg-slate-800 shadow-2xl transform translate-x-full lg:translate-x-0 lg:static lg:w-80 transition-transform duration-300 z-30 flex flex-col border-l dark:border-slate-700">
        ...
        <div class="flex-1 overflow-y-auto p-4 custom-scrollbar">
            <div class="grid grid-cols-5 gap-2" id="grid-nomor"></div>
        </div>
...
<script>
    ...
    function renderGrid() {
        ...
        stateSoal.forEach((s, i) => {
            ...
            if (i === currentIndex) cls += ' nav-active';
    ...
    function toggleNav() { document.getElementById('nav-sidebar').classList.toggle('translate-x-full'); }
    ...
    function cekSelesai() {
        ...
        if (adaRagu) {
            Swal.fire({
                ...
                text: 'Hilangkan tanda Ragu-ragu (Kuning) sebelum mengumpulkan.',
                icon: 'warning', confirmButtonText: 'Cek Lagi'
            });
            if(window.innerWidth < 1024) toggleNav();
            return;
        }
        ...
        if (adaKosong) msg = "Masih ada soal yang BELUM DIJAWAB. Yakin ingin menyelesaikan?";

        Swal.fire({
            title: 'Konfirmasi', text: msg, icon: adaKosong ? 'warning' : 'question',
            showCancelButton: true, confirmButtonText: 'Ya, Kumpulkan', confirmButtonColor: '#16a34a'
        }).then((result) => { if (result.isConfirmed) selesaiUjian(false); });
    }
    ...
    const timerInterval = setInterval(() => {
        ...
        if (dist < 0) { clearInterval(timerInterval); selesaiUjian(true); return; }
    ...
    function triggerWarning() {
        ...
        const fd = new FormData();
        fd.append('id_ujian_siswa', CONFIG.idUjianSiswa);
        fd.append('jenis', 'blur');
        fetch('<?= site_url('siswa/ujian/catatPelanggaran') ?>', {method:'POST', body:fd});
    }
    function kembaliKeUjian() { document.getElementById('warning-overlay').style.display = 'none'; }
</script>
<?= $this->endSection() ?>