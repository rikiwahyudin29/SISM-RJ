...
class Monitoring extends BaseController
{
    ...
    public function lihat($id_ruangan)
    {
        ...
        if(!$ruangInfo) return redirect()->to('guru/monitoring')->with('error', 'Ruangan tidak ditemukan.');
        ...
        // PERBAIKAN DISINI: Panggil view 'detail'
        return view('guru/monitoring/detail', $data); 
    }
    ...
    public function aksi_masal()
    {
        ...
        switch ($jenis_aksi) {
            case 'reset':
                ...
                break;
            case 'stop':
                $db->whereIn('id_siswa', $siswa_ids)->where('status !=', 'SELESAI')->update(['status' => 'SELESAI', 'waktu_selesai' => date('Y-m-d H:i:s')]);
