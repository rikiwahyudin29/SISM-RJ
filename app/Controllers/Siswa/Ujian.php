...
class Ujian extends BaseController
{
    ...
    public function index()
    {
        ...
        foreach($ujian as &$u) {
            ...
            // Ambil Log Terakhir (Hindari duplikat sesi)
    ...
    public function mulai()
    {
        ...
        if ($jadwal->setting_token == 1 && $token !== strtoupper($jadwal->token)) return redirect()->back()->with('error', 'Token Salah!');

        // Cek Sesi Existing (Ambil yang terbaru)
    ...
    public function kerjakan($idUjianSiswa)
    {
        ...
        // Merge Redis (Prioritas Cache jika ada)
    ...
    // 5. SIMPAN JAWABAN (Support Ragu-ragu)
    public function simpanJawaban()
    {
        ...
        // Merge agar data ragu tidak menimpa jawaban atau sebaliknya
        if(isset($curr[$idSoal])) {
            $newData = array_merge($curr[$idSoal], $newData);
        }
        $curr[$idSoal] = $newData; 
        $this->cache->save($key, $curr, 10800); 
    ...
    // 6. SELESAI UJIAN (FIX: MIN_WAKTU_SELESAI)
    public function selesaiUjian()
    {
        $idUjian = $this->request->getPost('id_ujian_siswa');
        $isAuto  = $this->request->getPost('is_auto'); // Flag dari JS jika waktu habis

        $sesi = $this->db->table('tbl_ujian_siswa')->where('id', $idUjian)->get()->getRow();
        $jadwal = $this->db->table('tbl_jadwal_ujian')->where('id', $sesi->id_jadwal)->get()->getRow();

        // --- [FIX LOGIKA MIN SUBMIT] ---
        // Jika bukan auto submit, cek kolom min_waktu_selesai dari DB
        if ($isAuto != '1') {
            $minMenit = $jadwal->min_waktu_selesai ?? 0; // Sesuai screenshot DB
            
            $waktuMulai = strtotime($sesi->waktu_mulai);
            $sekarang = time();
            $selisihMenit = floor(($sekarang - $waktuMulai) / 60);

            if ($selisihMenit < $minMenit) {
                return $this->response->setJSON([
                    'status' => 'error', 
                    'message' => 'Belum bisa selesai. Minimal pengerjaan: ' . $minMenit . ' menit. Anda baru berjalan ' . $selisihMenit . ' menit.'
                ]);
            }
        }
        ...
        foreach($list as $j) {
            ...
            if($sc==0 && (empty($inp)&&empty($opt))) $kos++; elseif($sc==0) $sal++; else $ben++;
            $nilai = $sc * $j['bobot']; 
            $skorTot += $nilai; 
            $bobotTot += $j['bobot'];
